    ...
    public class ReflectiveScript : MonoBehaviour
    {
        ...
        private bool ShouldUpdate()
        {
            try
            {
                ...
                else if (Planetarium.GetUniversalTime() >= this.lastUpdate + this.updateRate)
                {
                    this.dirty = 6;
                    return true;
                }
        ...
        private void UpdateCubemap(int faceMask)
        {
            if (!(bool)((UnityEngine.Object)this._cam))
            {
                ...
                //this._cam.cullingMask = (1 << (0xFFFFFFFF));
                this._cam.cullingMask &= ~(1 << 12);

                this._cam.depth = float.MaxValue;
                this._cam.farClipPlane = this.FarClipPlane;
                //this._cam.farClipPlane = Camera.main.farClipPlane;
                ...
                for (int i = 0; i < 32; i++)
                {
                    ...
                    {
                        case 4:
                            this._cam.layerCullDistances[i] = float.MaxValue;
                            break;
                        case 9:
                            this._cam.layerCullDistances[i] = float.MaxValue;
                            break;
