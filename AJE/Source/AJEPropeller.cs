...
//          public string velocityvector;
   ...
   //         r0 *= 0.3048f;
            ...
            propeller = new AJEPropellerSolver(r0, v0, omega0, rho0, power0);
            pistonengine = new PistonEngine(power, omega / gearratio);
        ...
        public void FixedUpdate()
        {
            ...
            propeller.calc(density, v/SpeedBuff, omega);

            
            
            pistonengine.calc(pressure, temperature, omega / gearratio);
            
            if (!useOxygen)
            {
                pistonengine._power = power;
                pistonengine._torque = power / omega;
            }
 ...
 //           velocityvector = ((int)v2.x).ToString() + " " + ((int)v2.y).ToString() + " " + ((int)v2.z).ToString() + " " + ((int)v2.magnitude).ToString();
    ...
    public class PistonEngine
    {
        ...
        public void setTurboParams(float turbo, float maxMP)
        {
            ...
            float T = 298f * (float)Math.Pow(P / P0, 2f / 7f);
            _rho0 = P / (287.1f * T);
        }
        public void calc(float pressure, float temp, float speed)
        {
            ...
            float rpm_factor = A * (float)Math.Pow(B, rpm_norm) * (float)Math.Pow(rpm_norm, C);
            ...
            float T = temp * (float)Math.Pow((_mp * _mp) / (pressure * pressure), 1.0 / 7.0);
            ...
            float specHeat = 1300;
            float corr = 1.0f / ((float)Math.Pow(_compression, 0.4f) - 1.0f);
    ...
    public class AJEPropellerSolver
    {
        ...
        public AJEPropellerSolver(float radius, float v, float omega, float rho, float power)
        {
            ...
            _lambdaPeak = (float)Math.Pow(1f / 9f, 1f / 8f);
            _beta = 1.0f / (_lambdaPeak - (float)Math.Pow(_lambdaPeak, 9));
        ...
        public void calc(float density, float v, float omega)
        {
            ...
            if (_manual)
                _j0 = _baseJ0 * (float)Math.Pow(2, 2 - 4 * _proppitch);
